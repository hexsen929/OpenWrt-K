...
function input_parameters() {
    ...
    while true; do
        ...
        OpenWrt_K_url="$(whiptail --title "输入存储库地址" --inputbox "$inputbox" 10 80 https://github.com/hexsen929/OpenWrt-K 3>&1 1>&2 2>&3)"
        exitstatus=$?
...
function import_ext_packages_config() {
    ...
    if [ $OPTION = 1 ]; then
        DOWNLOAD_URL=https://raw.githubusercontent.com/hexsen929/OpenWrt-K/main/config/default-extpackages.config
    elif [ $OPTION = 2 ]; then
